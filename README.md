# UPDATE
Уже через день после публикации этого бота на гитхаб, телеграм сделал новое право для админов на блокировку участников. Теперь вы можете использовать этого бота только как "права по шаблонам". 
# Зачем он нужен
...
### Пример
/setrole 1180538155 -1001595701195 admin
### Где взять айди
...
```
## Запускатор start.bat
```bat
@echo off
setlocal

set "APP_JAR=TelegramDefenderBot-1.0.jar"
set "JAVA_EXE=java"
...
rem Проверка наличия JAR
if not exist "%APP_JAR%" (
  echo JAR-файл "%APP_JAR%" не найден.
  pause
  exit /b 1
)

rem Запуск
"%JAVA_EXE%" -Xms128m -Xmx128m -Dfile.encoding=UTF-8 -jar "%APP_JAR%"
